	...
	<view class="content" :style="{height:pageHeight + 'px'}">
		...
		<view class="edit">
			<block  v-for="(item,index) in editItems" :key="index">
				<textarea :placeholder="editItems.length == 1 ? placeText : ''"  auto-height="true" :data-index="index" @input="inputIng" @linechange="linechage" v-if="item.type=='textarea'" :focus="index + 1 == editItems.length && index != 0 ? true : false" :value="item.value"></textarea> 
