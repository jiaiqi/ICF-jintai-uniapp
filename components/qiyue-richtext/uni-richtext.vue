	...
	<view>
		...
		<view class="popup popup-insert-text" v-show="showInsertTextPopup">
			...
			<view class="input-content">
				...
				<textarea style="width:98%;" maxlength="-1" v-model="textareaData"></textarea>
			</view>
		...
		<view class="content">
			<view class="placeholder-tip" @click="showInputPopup(-1)" v-if="list.length==0">请输入内容</view>
			<view ref="richtext" v-for="(item,index) in list" :key="index" :style="index==richListIndex?'background:#cce0f2;':''" @click="showInputPopup(index,item)">
							...
							uni.uploadFile({
								...
								success: (uploadFileRes) => {
									if (this.richListIndex == -1) {
										...
										this.list.splice(this.richListIndex + 1, 0, {
											"tagType": "image",
											"value": uploadFileRes.data,
											"style": ""
										});
			...
			joint(){
				...
				let htmlStr = ''
					this.list.forEach(item=>{
						htmlStr += `<${item.tagType} style='${item.style}'>${item.value}</${item.tagType}>`
					})
				this.$emit('listenChild',htmlStr)
			},
			reset(){
				//清除
				this.list = []
