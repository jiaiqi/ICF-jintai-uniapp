<template>
	<view class="add_pages ">
		...
		<view class="forumClassBox cu-form-group margin-top">
			...
			<picker @change="Picker1Change" :value="index2" :range="columnsList">
				<view class="picker">{{ index2 > -1 ? columnsList[index2] : '请选择栏目' }}</view>
			</picker>
		</view>
		<view class="forumClassBox cu-form-group margin-top">
...
export default {
	...
	methods: {
		...
		getColumns() {
			...
			let req = {
				...
				colNames: ['*'],
				condition: [{ colName: 'service_name', value: serviceName, ruleType: 'eq' }, { colName: 'use_type', value: 'detail', ruleType: 'eq' }],
			...
			this.$http.post(url, req).then(res => {
				if (res.data.data) {
					...
					columnArr.map(item => {
						columnList.push(item.label);
					});
					this.selectList = colArr;
					console.log(selectList,columnList);
					setTimeout(() => {
						this.setPickerData(selectList,columnList);
					}, 500);
				}
			});
		},
 getCategory() {
      ...
      let url = this.$api.select +'/' + this.appName+ '/select/'+this.query.service_name;
      let req = {
      ...
      this.$http
		...
		submitData() {
			...
			let column = this.columnsList[this.index2]?this.columnsList[this.index2]:' - '
			let type = '';
			selectList.map(item => {
			...
			console.log(content);
			let url = this.$api.add +'/' +this.appName+'/operate/srvzhsq_forum_note_add';
			let req = [
				{
		...
		setPickerData(classArr,colArr) {
			this.classify = classArr;
			this.columnsList = colArr
		}
	},
	onLoad(option) {
		if(option.query){
